...
impl LuaPickle {
    fn read(&mut self) -> Result<(u8, Value), Box<dyn std::error::Error>> {
        ...
        self.file.read_exact(&mut type_char)?;
        ...
        let content = match type_char {
            ...
            0x07 => {
                ...
                Value::Number(Number::from_f64(v).ok_or("not valid 64bit float")?)
            }
            0x08 => self.readstr(Strsize::Byte)?,
            0x09 => self.readstr(Strsize::Halfword)?,
            0x0b => {
                let mut buff = [0u8; 4];
                self.file.read_exact(&mut buff)?;
    ...
    fn readtab(&mut self, count: u32) -> Result<Value, Box<dyn std::error::Error>> {
        ...
        if count > 0 {
            for index in 1..count + 1 {
                let (_, coutent) = self.read()?;
                dict.insert(index.to_string(), coutent);
            }
        }
        loop {
            let (type_char, key) = self.read()?;
            if type_char == 0 {
                break;
            }
            let (_, coutent) = self.read()?;
            let k = match key {
                Value::Number(_) => key.as_u64().ok_or("cast fail")?.to_string(),
                Value::String(_) => key.as_str().ok_or("cast fail")?.to_string(),
                _ => key.to_string(),
            };
...
pub fn unpickle(filepath: &str, skipbyte: u64) -> Result<Value, Box<dyn std::error::Error>> {
    ...
    file.seek(SeekFrom::Start(skipbyte))?;

    let mut an_pickle = LuaPickle { file };
    let (_, dict) = an_pickle.read()?;
